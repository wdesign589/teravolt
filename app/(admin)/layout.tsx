...
export default function AdminLayout({
  ...
  // Force desktop viewport on mobile for admin dashboard
  useEffect(() => {
    ...
    // Add desktop viewport meta tag for admin dashboard
    const viewportMeta = document.createElement('meta');
    viewportMeta.name = 'viewport';
    viewportMeta.content = 'width=1280, initial-scale=0.3, maximum-scale=1.0, user-scalable=yes';
    document.head.appendChild(viewportMeta);
    
    // Cleanup function to restore responsive viewport when leaving admin dashboard
    return () => {
      const adminViewport = document.querySelector('meta[name="viewport"]');
      if (adminViewport) {
        adminViewport.remove();
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      <div className="flex">
        {/* Sidebar */}
        <aside className="w-64 min-h-screen bg-slate-900/50 backdrop-blur-xl border-r border-white/10">
          {/* Logo */}
          <div className="p-6 border-b border-white/10">
            <Link href="/admin" className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-emerald-400 text-xs">Teravolt Incorporated</div>
              </div>
            </Link>
          </div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 mb-3">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-emerald-400 text-xs font-medium">Administrator</div>
                </div>
              </div>
            </div>
            <button
              onClick={handleLogout}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg transition-all border border-red-500/30"
            >
